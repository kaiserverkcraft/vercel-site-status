...
// site env
const siteConfig = {
  siteTitle: process.env.SITE_TITLE || "kcraftnetwork 服务监测",
  siteDescription: process.env.SITE_DESCRIPTION || "一个简约的站点监测 | a simple site-status pages, powered by uptimerobot",
  siteKeywords: process.env.SITE_KEYWORDS || "kakcraft,kcraftnetwork,kaivps,kaialist,kcraftnetwork 服务器监测",
  siteLogo: process.env.SITE_LOGO || "/favicon.ico",
  siteIcp: process.env.SITE_ICP || "",
  countDays: Number(process.env.COUNT_DAYS || 60),
  // SHOW_LINKS env: when set to "true" will enable showing site links.
  // Default is false when not provided.
  showLinks: process.env.SHOW_LINKS || false,
